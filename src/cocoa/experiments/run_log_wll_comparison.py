...
# Since this is now inside src/cocoa/experiments, we don't need sys.path hacks if run as module
# But for direct execution, we might. Best to rely on standard imports.
...
def run_log_comparison():
    ...
    # Use standard default or specific if needed
    ...
    log_runner = GeneralizedComboExperimentRunner(
        ...
        poly_order=1,
        save_results=True,
        # output_base_dir removed to use default "output/cocoa_forecast"
    )
    ...
    std_runner = ConvexComboExperimentRunner(
        combo_type="NP",
        model_name="NP_Standard_Compare",
        feature_cols=DEFAULT_FEATURE_COLS,
        target_col=DEFAULT_TARGET_COL,
        data_path=PROCESSED_DATA_PATH,
        oos_start_date=DEFAULT_OOS_START_DATE,
