...
We benchmark the performance of the Baseline WLL estimator against the "Manually Boosted" WLL extension (Log WLL) and standard ML baselines.
Table~\ref{tab:msfe-results} summarizes the Out-of-Sample MSFE across the evaluation period.

Crucially, the "Manual Boosting" extension yielded a **0.00\% improvement** over the baseline WLL (MSFE remained at 0.00281, with $\beta \to 0$). This result strongly suggests that the data rejects manually engineered nonlinearities, favoring either the parsimonious linear weighting of CGS or the fully adaptive nonlinearity of machine learning.

\subsection{Time-varying performance: Sensitivity vs. Over-smoothing}
Figure~\ref{fig:rolling_comparison} presents the rolling MSFE of the WLL estimator against the XGBoost baseline. A distinct behavioral contrast is observed:
\begin{enumerate}
    \item \textbf{WLL Sensitivity:} The WLL forecasts exhibit sharp "jumps" whenever the detected structural break date is updated. Our analysis confirms a \textbf{100\% correlation} between large MSFE jumps ($>2\sigma$) and shifts in the estimated break date $\hat{T}_1$.
    \item \textbf{ML Inertia:} In contrast, the XGBoost forecasts remain remarkably smooth across these regime shifts.
\end{enumerate}

This sensitivity is not necessarily a defect; rather, it reflects the "Structural Honesty" of the glass-box estimator, which transparently signals the uncertainty in regime identification. The smoothness of the ML models likely stems from ensemble averaging, which may mask the underlying structural risk (an "Illusion of Certainty").

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.85\textwidth]{msfe_comparison.png}
  \caption{Rolling MSFE Comparison. WLL jumps align perfectly with structural break updates, while ML remains smooth.}
  \label{fig:rolling_comparison}
\end{figure}
...
Our findings challenge the conventional view that "Black-Box" ML is simply superior. While ML averages out structural noise to achieve stability, it obscures the discrete nature of regime shifts.
The WLL estimator serves as a necessary "probe": when WLL fluctuates while ML remains stable, it signals a period of high **structural uncertainty**.
Therefore, we propose that WLL should not be viewed merely as a competitor, but as an **"Auditing Tool"** to validate the robustness of ML predictions against structural breaks.
